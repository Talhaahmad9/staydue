  ...
  return (
    <div className="mt-4 pt-4 border-t border-line/50">
      <div className="flex items-center gap-3 mb-3">
        <p className="text-xs uppercase tracking-widest font-medium text-text-muted">Overdue</p>
        <span className="bg-urgency-today border border-urgency-todayBorder text-urgency-todayText text-xs font-medium px-2 py-0.5 rounded-full">
          {overdueDeadlines.length}
        </span>
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="ml-auto text-text-muted hover:text-text-primary transition-colors"
          aria-label={isExpanded ? "Collapse overdue" : "Expand overdue"}
        >
          <svg
