...
interface ProExpiringEmailProps {
  userName: string;
  expiresAt: Date;
}

export default function ProExpiringEmail({ userName, expiresAt }: ProExpiringEmailProps) {
  const expiryStr = expiresAt.toLocaleDateString("en-PK", {
  ...
  return (
    <Html lang="en">
      <Head />
      <Preview>Your StayDue Pro expires tomorrow — renew to stay uninterrupted</Preview>
      <Body
        style={{
        ...
        <Container style={{ maxWidth: "600px", margin: "0 auto", padding: "24px" }}>
          ...
          <Section style={{ marginBottom: "24px" }}>
            ...
            <Text style={{ fontSize: "15px", color: "#D4D4D4", margin: "0 0 20px 0", lineHeight: "1.6" }}>
              Your <strong style={{ color: "#F5F5F5" }}>StayDue Pro</strong> subscription expires
              on <strong style={{ color: "#F5F5F5" }}>{expiryStr}</strong>. After that, WhatsApp
              reminders will pause — email reminders will continue.
            </Text>
            <Text style={{ fontSize: "14px", color: "#A3A3A3", margin: "0 0 20px 0", lineHeight: "1.6" }}>
              Submit a new payment from your settings to extend your subscription before it expires.
            </Text>
