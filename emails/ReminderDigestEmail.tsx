...
function getCardBorderColor(interval: BatchDeadlineItem["interval"]): string {
  switch (interval) {
    case "3-day":
      return "#0D9488";
    case "1-day":
...
function getIntervalLabel(interval: BatchDeadlineItem["interval"]): string {
  switch (interval) {
    case "3-day":
      return "3 days left";
    case "1-day":
...
function getIntervalLabelColor(interval: BatchDeadlineItem["interval"]): string {
  switch (interval) {
    case "3-day":
      return "#0D9488";
    case "1-day":
...
export default function ReminderDigestEmail({ payload }: ReminderDigestEmailProps) {
  const { userName, deadlines } = payload;
  const count = deadlines.length;
  const previewText = count === 1
    ? `Reminder: ${deadlines[0].title}`
      ...
      >
        <Container style={{ maxWidth: "600px", margin: "0 auto", padding: "24px" }}>
          ...
          {deadlines.map((deadline, index) => {
            const borderColor = getCardBorderColor(deadline.interval);
            const labelColor = getIntervalLabelColor(deadline.interval);
            const label = getIntervalLabel(deadline.interval);
